...
@app.route('/setup')
def setup_guide():
    with open('docs/SETUP_GUIDE_HINDI.md', 'r') as file:
        content = file.read()
    return f"""
    <!DOCTYPE html>
    <html>
...
@app.route('/hosting')
def hosting_guide():
    with open('docs/HOSTING_GUIDE_HINDI.md', 'r') as file:
        content = file.read()
    return f"""
    <!DOCTYPE html>
    <html>
...
@app.route('/download')
def download():
    return send_from_directory('static', 'ai-news-scraper-auto-blogger-pro.zip', as_attachment=True)